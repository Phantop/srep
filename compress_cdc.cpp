...
BYTE **zpaq_find_chunks (BYTE* &ptr, BYTE* pend, BYTE* buf, BYTE* bufend, BYTE** lastmark, unsigned L, unsigned MIN_MATCH)
{
  ...
  for (BYTE *p = (ptr-buf>8000? ptr-8000 : buf), *lastp=p;  likely (p<pend);  p++)
  {
    ...
    {
      // Note: the first chunk in the stripe may be less than MIN_MATCH bytes, but anyway will be filtered out just prior to ENCODE_LZ_MATCH
      if (p>ptr)  *lastmark++ = p;
      lastp = p;
      c1 = hash = 0;
...
struct CDC_Thread
{
  ...
  {
    BYTE *vhash  =  vhashes + 2*VMAC_TAG_LEN_BYTES;     // the first vhashes[] slot is reserved for the chunk split between the current and previous stripes
...
void compress_CDC (bool ZPAQ_CDC, unsigned L, unsigned MIN_MATCH, Offset block_start, HashTable &h, CDC_Global &g, char *_buf, int block_size, unsigned &literal_bytes, STAT *statbuf, STAT *&stat)
{
  ...
  for (BYTE *ptr=buf; free_jobs<g.Threads.NumThreads || ptr<bufend; free_jobs++)
  {
    ...
    BYTE *vhash = job.t->vhashes,  **curmark = job.t->marks;
    if (curmark < job.t->lastmark)   // compute hash of chunk split between the current and previous stripes (or first chunk of the block)
      job.t->compute_single_chunk_hash (last_chunk, *curmark, vhash);
    for (;  curmark < job.t->lastmark;  curmark++)
    {
