...
// Try to alloc with and without MEM_TOP_DOWN flag in order to work around Win8.1 problem
void* TryVirtualAlloc (int64 size, DWORD AllocationType)
{
  void* address = ::VirtualAlloc(0, size, AllocationType | MEM_COMMIT | (AllocTopDown? MEM_TOP_DOWN : 0), PAGE_READWRITE);
  if (address == NULL  &&  AllocTopDown) {
    alloc_debug_printf((stderr, "  MEM_TOP_DOWN failed:"));
    address = ::VirtualAlloc(0, size, AllocationType | MEM_COMMIT, PAGE_READWRITE);
  }
  return address;
}
...
void *BigAlloc (int64 size, LPType LargePageMode) throw()
{
  ...
  if (LargePageMode!=DISABLE)
  {
    ...
    if (page  &&  (size>=page || LargePageMode==FORCE))
      address = TryVirtualAlloc (roundUp(size,page), MY_MEM_LARGE_PAGES);
    if (address==0)
      alloc_debug_printf((stderr, "  failed\n", int(page)));
  }

  if (address==0 && LargePageMode!=FORCE)
    address = TryVirtualAlloc (size, 0);
...
LongMemSize parseMem64 (char *param, int *error, char spec)
{
  uint64 n=0;
  char c = *param=='='? *++param : *param;
...
char *showMem (MemSize mem, char *result, bool add_b)
{
  const char *b = (add_b? "b" : "");
       if (mem   ==0) sprintf (result, "0%s", b);
  else if (mem%gb==0) sprintf (result, "%.0lfg%s", double(mem/gb), b);
  else if (mem%mb==0) sprintf (result, "%.0lfm%s", double(mem/mb), b);
  else if (mem%kb==0) sprintf (result, "%.0lfk%s", double(mem/kb), b);
  else                sprintf (result, "%.0lf%s",  double(mem), b);
  return result;
}

// Returns a string with the amount of memory
char *showMem64 (LongMemSize mem, char *result, bool add_b)
{
  const char *b = (add_b? "b" : "");
       if (mem   ==0) sprintf (result, "0%s", b); else
  if(mem%terabyte==0) sprintf (result, "%.0lft%s", double(mem/terabyte), b);
  else if (mem%gb==0) sprintf (result, "%.0lfg%s", double(mem/gb), b);
  else if (mem%mb==0) sprintf (result, "%.0lfm%s", double(mem/mb), b);
  else if (mem%kb==0) sprintf (result, "%.0lfk%s", double(mem/kb), b);
  else                sprintf (result, "%.0lf%s",  double(mem), b);
...
#if !defined(__GNUC__)  ||  (__GNUC__>=4  &&  !defined(GHC_GCC))
...
void Taskbar_SetWindowNormal(HWND hWnd)                               {DefaultTaskbar.SetProgressState(hWnd, TBPF_NORMAL);}
void Taskbar_SetWindowError (HWND hWnd)                               {DefaultTaskbar.SetProgressState(hWnd, TBPF_ERROR);}
void Taskbar_SetWindowPause (HWND hWnd)                               {DefaultTaskbar.SetProgressState(hWnd, TBPF_PAUSED);}
